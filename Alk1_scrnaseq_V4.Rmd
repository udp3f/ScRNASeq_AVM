...
This is the version for the final manuscript
...
library(rhdf5)
#Read 10X hdf5 file
data <- Read10X_h5("~/CONTROL/filtered_feature_bc_matrix.h5", use.names = TRUE, unique.features = TRUE)
...
library(rhdf5)
#Read 10X hdf5 file
data <- Read10X_h5("~/CONTROL-CDKi/filtered_feature_bc_matrix.h5", use.names = TRUE, unique.features = TRUE)
...
library(rhdf5)
#Read 10X hdf5 file
data <- Read10X_h5("~/KO/filtered_feature_bc_matrix.h5", use.names = TRUE, unique.features = TRUE)
...
library(rhdf5)
#Read 10X hdf5 file
data <- Read10X_h5("~/KO-CDKi/filtered_feature_bc_matrix.h5", use.names = TRUE, unique.features = TRUE)
...
#Tip, venous ("Ephb4","Nrp2","Nr2f2","Aplnr","Ptgs1"),capillary ("Slc7a5","Mfsd2a","Rgcc","Abcb1a","Prkcb"), arterial ("Sox17","Efnb2","Gja5","Gja4","Bmx")
...
# https://www.sciencedirect.com/science/article/pii/S0006291X21000784?via%3Dihub; https://www.proteinatlas.org/ENSG00000166165-CKB/single+cell+type; http://mousebrain.org/adolescent/genesearch.html
#http://mousebrain.org/genes/C1qb.html
...
obj_CR <- subset(x=integrated_subset.new, idents = c("Capillary","Venous","Proliferative","Tip","Arterial")) ##10,11 clusters could not be resolved, so excluding from analysis
...
DimPlot(obj_CR, reduction = "phate", cells = cells)
...
wrap_plots(p1,p2,p3,p4,guides='collect') + plot_annotation(title = "Venous markers") & theme (plot.title = element_text(size = 25)) # removing d
...
wrap_plots(p1,p2,p3,p4,guides='collect') + plot_annotation(title = "Arterial markers") & theme (plot.title = element_text(size = 25))
...
### eG1 (Nick's)
dat2 <- readLines("~/EarlyG1genes.csv")
eG1 <- list(dat2)
...
#dated July 24 2023
genes <- c("Acvrl1","Smad5","Smad1","Smad4","Eng")
DotPlot(data, assay="RNA", features = genes, dot.scale = 6, group.by = "orig.ident", idents="Venous") + RotatedAxis()

#dated July 24 2023
genes <- c("Tgfbr1","Tgfbr2","Smad6","Tgfb1","Tgfb2")
DotPlot(data, assay="RNA", features = genes, dot.scale = 6, group.by = "orig.ident", idents="Arterial") + RotatedAxis()

#dated July 24 2023
genes <- c("Dlg1","Igf1r","Pik3c2b","Setx","Sox9","Tgfb1","Arfgef1","Fgfr1","Itgb1bp1","Mtor","Mul1","Mydgf", "Nts","Phb","Phlda3","Rara","Rictor")
DotPlot(data, assay="RNA", features = genes, dot.scale = 6, group.by = "orig.ident") + RotatedAxis()
...
## remove CT_CDKi; using this image for lab webpage
data <- subset(x=obj_CR, subset = orig.ident %in% c("CT","KO","KO_CDKi"))
#change labels for orig.ident
current.ids <- c("CT","KO","KO_CDKi")
new.ids <- c("Control", "Disease", "Treatment")
data$orig.ident <-  plyr::mapvalues(x = data$orig.ident, from = current.ids, to = new.ids)
data$orig.ident  <- droplevels(data$orig.ident)

library(gganimate)
p <- ggplot(data = data[cells,]) + cowplot::theme_cowplot() # initialize plot
p + geom_point(aes(x = PHATE_1, y = PHATE_2), group = seq_along(data[cells,]$orig.ident)) 
# color by arterial identity module scores art_ident1
plot <- p + geom_point(aes(x = PHATE_1, y = PHATE_2, color = !!data[cells,]$art_iden1)) + scale_color_gradientn(colors = c("lightgrey", "blue")) + guides(color = guide_colorbar(title = 'Module scores'))
# animate expression across different conditions with transition states set to the orig.ident
myplot <- plot + transition_states(data[cells,]$orig.ident, transition_length = 1, state_length = 1)  +  enter_fade() + exit_shrink() +  ease_aes('sine-in-out') + ggtitle('{closest_state}')
#save image for lab webpage
library(gifski)
animate(myplot, duration = 5, renderer = gifski_renderer())
anim_save("arterial.gif")

```


# Miscellaneous - Module scores for arterial identity, late G1, BMP/vegf/migration signaling cell cycle; 
```{r, eval=FALSE}
...
dat3 <- readLines("~/LateG1genes.csv")
lG1 <- list(dat3)
...
# checked every individual gene for the final list that goes for module scoring (removed Bmp2,Stc1,Dcxr,Rflna,Myo1e,Rgs5,Plcl1,Pdlim2,Herc6); The following features are not present in the object: Fyn, Smpdl3a, B2m, Thbd, Icam2, Picalm, Gnpda2, Sh3bgrl, Cavin1, not searching for symbol synonyms
